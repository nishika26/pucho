...
async def run(state: State, runtime: Runtime[RouterContext]) -> dict[str, Any]:
    ...
    model = ChatOpenAI(
        model=os.environ.get("PUCHO_LEGAL_MODEL", "gpt-4o-mini"),
    ...
    result = await agent.ainvoke(
        ...
        config={"configurable": {"thread_id": f"legal-{user_id}"}},  # Use user_id for uniqueness
    )
    ...
    # Hand back output_text for the persist_message node, plus BOTH sides of this
    # turn (user question + bot reply) so the add_messages reducer appends them to
    # the running short-term history — next turn's history then includes what the
    # user actually asked, not just the bot's answers.
    return {
