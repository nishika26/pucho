...
Retrieves from the legal corpus, injects the user's stored legal facts into
the system prompt, answers the user's question, and produces a voice reply
(TTS → Vercel Blob upload) when the user's original modality was voice.
...
SYSTEM_PROMPT = """You are the legal-domain assistant for Pucho, a WhatsApp chatbot.

Rules:
- Answer ONLY from the retrieved legal documents provided to you.
- If the documents do not contain the answer, say so plainly and suggest the
  user consult a qualified lawyer. Do not invent statutes, case law, or advice.
- Do NOT provide medical or financial advice even if asked.
...
- When the user mentions personal facts relevant to legal advice (property,
  marital status, ongoing cases, etc.), those are surfaced separately as
  "Known facts" — use them, do not re-ask.
"""

_retriever = get_retriever("legal")
...
async def run(state: State, runtime: Runtime[RouterContext]) -> dict[str, Any]:
    ...
    memories = await crud_memory.list_for_user_domain(user_id, MemoryDomain.LEGAL)
    ...
    model = ChatOpenAI(model=os.environ.get("PUCHO_LEGAL_MODEL", "gpt-4o-mini"))
    # No tools: TTS is handled in code after the LLM produces text, which
    # avoids the bytes-in-ToolMessage coercion problem and a redundant LLM
    # roundtrip just to trigger TTS.
    agent = create_agent(
        model=model,
        tools=[],
    ...
    result = await agent.ainvoke(
        ...
        config={"configurable": {"thread_id": "pucho-legal"}},
    )
    ...
    if modality == "voice":
        output_audio_url = await synthesize_and_upload(
            final_text, language=language, prefix="legal"
        )

    # Hand back output_text for the persist_message node, plus a chat-history
    # entry so the reflect node can read the latest exchange.
    return {
